...
#  DecodificatoreMessaggio. Il costruttore dovrebbe ricevere un numero intero chiamato chiave.
#  Si definisca il metodo codifica(testoInChiaro) così che ogni lettera del testo sia spostata dal valore
#  contenuto in chiave.
...
#      Suggerimento: si potrebbe definire un metodo privato sposta_carattere(c) che restituisca la codifica di un
#  singolo carattere c da concatenare agli altri per costruire il messaggio codificato nel metodo
#  codifica(testoInChiaro).
...
#  si divide il messaggio a metà e poi si prendono i caratteri da ognuna delle metà in modo alternato. Per esempio,
#  se il messaggio è 'abcdefghi', le metà sono 'abcde' e 'fghi' (nel caso in cui la lunghezza del testo da decifrare
#  sia un numero dispari, la prima metà deve essere più lunga della seconda metà).
...
# Si scriva il metodo decodifica(testoCodificato) per ognuna delle classi CifrarioAScorrimento e
#  CifrarioACombinazione.
...
#     Suggerimento: definire il metodo privato decodifica_combinazione() per la classe CifrarioACombinazione
#  che compie un'azione inversa al metodo combinazione().
